  ...
  await Promise.all(
    commands.map(
      (
        ...
        return authedFetch(
          `https://discord.com/api/v8/applications/${applicationID}/guilds/${guildID}/commands/${command.id}`,
          {
            method: "DELETE",
          }
        );
...
export const setup = ({
  ...
  return async (): Promise<Response> => {
    try {
      ...
      return await createCommands({ applicationID, guildID, commands }, authedFetch);
    } catch {
      return new Response(
